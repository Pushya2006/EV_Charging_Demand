...
# Title
st.markdown("""
    <div style='text-align: center; font-size: 100px; font-weight: bold; color: #4f0303; margin-top: 20px;'>
...
# Subtitle
st.markdown("""
    <div style='text-align: center; font-size: 80px; font-weight: bold; padding-top: 10px; margin-bottom: 25px; color: #4f0303;'>
...
# Marquee
st.markdown("""
    <marquee behavior="scroll" direction="left" style="color:#000000; font-size:50px; font-weight:bold; background-color:#BFB0B3; padding:10px;">
        🚗 EV FORECASTING 🔋
    </marquee>
""", unsafe_allow_html=True)
...
st.image("EV_car.jpeg", width=800, use_container_width=False)

# Instruction line (darkblue)
st.markdown("""
    <div style='text-align: left; font-size: 22px; padding-top: 10px; color: darkblue;'>
...
# Cumulative Forecast header (darkblue)
st.markdown(f"""
    <h3 style='color:darkblue;'>📊 Cumulative EV Forecast for {county} County</h3>
""", unsafe_allow_html=True)
...
# Compare header (darkblue)
st.markdown("""
    <h2 style='color:darkblue;'>Compare EV Adoption Trends for up to 5 Counties</h2>
""", unsafe_allow_html=True)

# Multiselect label (darkblue)
st.markdown("""
    <div style='font-size: 18px; font-weight: bold; color: darkblue; padding-bottom: 5px;'>
        Select up to 5 counties to compare
    </div>
""", unsafe_allow_html=True)

multi_counties = st.multiselect("", county_list, max_selections=5)
...
if multi_counties:
    ...
    ax.set_title("EV Adoption Trends: Historical + 3-Year Forecast", fontsize=16, color='pink')
    ax.set_xlabel("Date", color='pink')
    ax.set_ylabel("Cumulative EV Count", color='pink')
    ax.grid(True, alpha=0.3)
    ax.set_facecolor("#0b1111")
    fig.patch.set_facecolor("#3b0000")
    ...
    growth_summaries = []
    for cty in multi_counties:
        sub = comp_df[comp_df['County'] == cty].reset_index(drop=True)
        hist_tot = sub['Cumulative EV'].iloc[len(sub) - forecast_horizon - 1]
        fore_tot = sub['Cumulative EV'].iloc[-1]
        if hist_tot > 0:
            growth_summaries.append(f"{cty}: {((fore_tot - hist_tot)/hist_tot)*100:.2f}%")
        else:
...
# Footer (darkblue)
st.markdown("""
    <div style='color:darkblue; font-weight:bold; font-size: 16px; padding-top:10px;'>
